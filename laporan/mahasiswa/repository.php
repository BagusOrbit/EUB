<?php 
	...
	class Repository{
		...
		{
			$query = $this->dbh->prepare("SELECT DISTINCT nama,fakultas,users.kode_user FROM jawaban JOIN users ON jawaban.kode_user = users.kode_user ORDER BY users.fakultas");
			$query->execute();
